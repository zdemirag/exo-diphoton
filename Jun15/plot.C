void plot() {
  ...
  */
  ...
  TH1F *h1 = new TH1F("h1","h1",binning,low,binning);
  h1->Sumw2();
  ...
  TH1F *h2 = new TH1F("h2","h2",binning,low,binning);
  h2->Sumw2();
  ...
  TH1F *h3 = new TH1F("h3","h3",binning,low,binning);
  h3->Sumw2();
  ...
  TH1F *h4 = new TH1F("h4","h4",binning,low,binning);
  h4->Sumw2();
  T->Draw("mass>>h4","weight","goff");

  TH1F *h = new TH1F("h","h",binning,low,binning);
  h->Sumw2();
  ...
  h->GetYaxis()->SetTitle("Events / "+Rebinning+" GeV");
  ...
  TF1 *fdijet1 = new TF1("fdijet1","[0]*(1-(x/8000))^[1] / (x/8000)^([2]+[3]*log(x/8000))",start,2500);
  ...
  std::cout<<"About to do the dijet2" << std::endl;
  TF1 *fdijet2 = new TF1("fdijet2","[0]*(1-(x/8000))^[1] / (x/8000)^([2]+[3]*log(x/8000))",500,2500);
  fdijet2->SetParameters(1e-5,7.2,5,0.1);
  fdijet2->SetLineWidth(2);
  fdijet2->SetLineColor(2);
  if(dijet){
      h->Fit(fdijet2,"MR");
      TF1 *f4 = new TF1("f4","[0]*(1-(x/8000))^[1] / (x/8000)^([2]+[3]*log(x/8000))",300,4000);
      f4->SetParameters(fdijet2->GetParameter(0), fdijet2->GetParameter(1),fdijet2->GetParameter(2),fdijet2->GetParameter(3),fdijet2->GetParameter(4));
  }




  TF1 *fbh = new TF1("fbh","[0]*(1+(x/8000))^[1] / (x/8000)^([2]+[3]*log(x/8000))",start,2500);
  fbh->SetParameters(1e-5,7.2,5,0.1);
  fbh->SetLineWidth(2);
  fbh->SetLineColor(2);
  if(bh){
    h->Fit(fbh,"MR");
    TF1 *f2 = new TF1("f2","[0]*(1+(x/8000))^[1] / (x/8000)^([2]+[3]*log(x/8000))",300,4000);
    f2->SetParameters(fbh->GetParameter(0), fbh->GetParameter(1),fbh->GetParameter(2),fbh->GetParameter(3));
  }
  
  TF1 *fzprime = new TF1("fzprime","pow(x,[0])*TMath::Exp([1] + [2]*x + [3]*x*x )",start,2500);
  fzprime->SetParameters(-4,10,0,0);
  fzprime->SetLineWidth(2);
  fzprime->SetLineColor(2);
  if(zprime){
    h->Fit(fzprime,"MR");
    TF1 *f3 = new TF1("f3","pow(x,[0])*TMath::Exp([1] + [2]*x + [3]*x*x )",300,4000);
...
f4->Draw("SAMEN");
  ...
  leg->AddEntry(f1,"Dijet Function" ,"l");
  //leg->AddEntry(f2,"Black Hole Function" ,"l");
  //leg->AddEntry(f3,"Zprime Function" ,"l");
  leg->AddEntry(f4,"Dijet Function 500 GeV Fit" ,"l");
  leg->SetFillColor(0);
...
//       Pull->SetMaximum(3.5);
//       Pull->SetMinimum(-3.5);
//       Pull->GetYaxis()->SetTitle("#sigma(Bkg-Fit)"); 
//     }
//   }
  ...
  if(ratio){
    ...
    Pull->SetMaximum(1.5);
    Pull->SetMinimum(-1.5);
    Pull->GetYaxis()->SetTitle("(Bkg-Fit)/Fit");
  
      */
      
      for(int i = 0; i<h->GetNbinsX(); i++){
          if(h->GetBinError(i) > 0 && h->GetBinCenter(i)> 500) 
          {
              double divided  = f1->Eval(h->GetBinCenter(i)) / f4->Eval(h->GetBinCenter(i));
              std::cout << "i :" << i<< " f1: " << f1->Eval(h->GetBinCenter(i)) <<" f4: " << f4->Eval(h->GetBinCenter(i)) << std::endl;          
              std::cout << divided << std::endl;
              Pull->SetBinContent(i,divided);
      ...
      Pull->SetMaximum(1.2);
      Pull->SetMinimum(0.2);
      Pull->GetYaxis()->SetTitle("Dijet / Z' ");
...
//c->SaveAs("bkgFit_ratio_Binning1.pdf");
  ...
  t->DrawLatex(0.65,0.65,"N_{MC} in range: 2184.47 Events ");
  //zprime
  //t->DrawLatex(0.65,0.60,"N_{Fit} in range: 2187.15 Events ");
  //bh
  //t->DrawLatex(0.65,0.60,"N_{Fit} in range: 2168.08 Events ");
  //dijet 
  //t->DrawLatex(0.65,0.60,"N_{Fit} in range: 2180.02 Events ");
  //dijet Range-300-2700
  //t->DrawLatex(0.65,0.60,"N_{Fit} in range: 2180.07 Events ");
  //dijet Range-300-2900
  //t->DrawLatex(0.65,0.60,"N_{Fit} in range: 2180.05 Events ");
  //rebinning 1
  t->DrawLatex(0.65,0.60,"N_{Fit} in range: 2184.13 Events ");
  //rebinning 2
  //t->DrawLatex(0.65,0.60,"N_{Fit} in range: 2283.82  Events ");
