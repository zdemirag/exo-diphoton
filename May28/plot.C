void plot() {
  ...
  */
  ...
  TFile *file1 = new TFile("/afs/cern.ch/work/z/zdemirag/work/Diphoton/CMSSW_5_3_2_patch4/src/May28/histograms_diphoton_tree_MC_all.root","READ");
  TFile *file2 = new TFile("/afs/cern.ch/work/z/zdemirag/work/Diphoton/CMSSW_5_3_2_patch4/src/May28/histograms_ExoDiPhotonAnalyzer_PFDec14th_DataABCD_GammaJet.root","READ");
  TFile *file3 = new TFile("/afs/cern.ch/work/z/zdemirag/work/Diphoton/CMSSW_5_3_2_patch4/src/May28/histograms_ExoDiPhotonAnalyzer_PFDec14th_DataABCD_JetJet.root","READ");
  TFile *file4 = new TFile("/afs/cern.ch/work/z/zdemirag/work/Diphoton/CMSSW_5_3_2_patch4/src/May28/histograms_ExoDiPhotonAnalyzer_PFDec14th_DataABCD.root","READ");

  TH1F *h = (TH1F*)file1->Get("h_Diphoton_Minv_FineBinning");
  TH1F *h2 = (TH1F*)file2->Get("h_GammaJet_minv_FineBinning");
  TH1F *h3 = (TH1F*)file3->Get("h_JetJet_minv_FineBinning");
  TH1F *h4 = (TH1F*)file4->Get("h_Diphoton_Minv_FineBinning");
  ...
  TF1 *fdijet1 = new TF1("fdijet1","[0]*(1-(x/8000))^[1] / (x/8000)^([2]+[3]*log(x/8000))",320,3000);
  ...
  if(pull){   
    for(int i = 0; i<h->GetNbinsX(); i++){
      if(h->GetBinError(i) > 0) Pull->SetBinContent(i,(h->GetBinContent(i) - f1->Eval(h->GetBinCenter(i))) / h->GetBinError(i));
  ...
  if(ratio){
    ...
    // This is the ratio
    for(int i = 0; i<h->GetNbinsX(); i++){
      if(h->GetBinError(i) > 0) Pull->SetBinContent(i,(h->GetBinContent(i) - f1->Eval(h->GetBinCenter(i))) / f1->Eval(h->GetBinCenter(i)));
      else Pull->SetBinContent(i,0);
    }
    Pull->SetMaximum(1.5);
    Pull->SetMinimum(-1.5);
  ...
  for(int i = 0; i<h4->GetNbinsX(); i++){
      std::cout <<"i:" << i  <<" h4->GetBinCenter(i)): " << h4->GetBinCenter(i) << " h4->GetBinContent(i): "  << h4->GetBinContent(i) <<std::endl;
      std::cout <<"i:" << i  <<" f1->Eval(h4->GetBinCenter(i)): " << f1->Eval(h4->GetBinCenter(i)) << std::endl;
    if(i>7 && i<76){
      sum += f1->Eval(h4->GetBinCenter(i)); 
      std::cout << "SUM: " << sum << std::endl;
    }
  }
  std::cout << "Integral for Data between 320 and 3 TeV: " << h4->Integral(8,75) << std::endl;
  std::cout << "Integral for MC between 320 and 3 TeV: " << h->Integral(8,75) << std::endl;
  std::cout << "Integral for Fit between 320 and 3 TeV: " <<  sum << std::endl;
  ...
  t->DrawLatex(0.65,0.75,"Fit range: 320 GeV - 3 TeV");
  t->DrawLatex(0.65,0.70,"N_{Data} in range: 1626 Events");
  t->DrawLatex(0.65,0.65,"N_{MC} in range: 1412.88 Events ");
  t->DrawLatex(0.65,0.60,"N_{Fit} in range: 1411.05 Events ");
